...
class Benchmark:
  ...
  def load_images_by_model(self, model, file_format='png'):
    ...
    filenames = sorted(glob.glob(os.path.join(self.path, '*' + '.' + file_format)))
  ...
  def save_images(self, images, log_path, iteration):
    ...
    for output, lr, hr, name in zip(images, self.images_lr, self.images_hr, self.names):
      # Save output
      path = os.path.join(log_path, self.name, '%d' % iteration)+'/SR/'+str(count)+'.png'
      self.save_image(output, path)
      # Save ground truth
      path = os.path.join(log_path, self.name, '%d' % iteration)+'/HR/'+str(count)+'.png'
      self.save_image(hr, path)
      # Save low res
      #path = os.path.join(log_path, self.name, '%d' % iteration)+'/LR/'+str(count)+'.png'
